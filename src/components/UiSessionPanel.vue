    ...
    <div class="session-panel-footer" @mousedown.stop="">
      <div class="session-tool-warp">
        ...
        <label class="session-tool-item">
          ...
          <input type="file" accept="image/png, image/jpg, image/jpeg" @change="imgFileChange">
        </label>
        <label class="session-tool-item">

          <i class="el-icon-folder-opened" style="font-size:25px ; position: relative;top:3px;"></i>
    ...
    methods:{
     ...
     async upload(){
        ...
        await this.$axios.post('http://localhost:3000/upload',formData)
                ...
                const size=Math.floor(this.file.size/1024/1024)+'MB'
                this.sendMessage(this.html,`file`,this.file.name,size)
              })
                .catch(()=>{this.$message.error('上传失败')});
      },

      sendMessage(content,type,filename,filesize){
        this.$emit("sendMessage",content,type,this.session,filename,filesize)
      }
    }
  }
</script>
