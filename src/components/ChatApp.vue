    ...
    methods:{
      ...
      addSessionMessage(message,sessionId){
        ...
        if (message.type === 'file'){
          message.content =  `/assets/images/${message.filename}`
          console.log(message)
        }
      ...
      sendMessage(content,type,session,filename,filesize){
        const message={
          from:this.loginUser,
          to:session,
          content:content,
        ...
        if(this.socket){
          this.socket.emit("message",message.from,message.to,message.content,message.type,message.filename,message.filesize)
        }
      },
      ...
      listenerMessage(from,to,message,type,filename,filesize){
        const _this=this;
        let isRead=false;
        if(to.type==='group'){
