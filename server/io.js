...
const util = {
  async login(user, socket, isReconnect) {
    ...
    const deviceType = this.getDeviceType(
      socket.handshake.headers["user-agent"].toLowerCase()
    );
    user.ip = ip;
  ...
  async loginSuccess(user, socket) {
    ...
    socket.on("message", (from, to, message, type, filename, filesize) => {
      if (to.type === "user") {
        if (type === "file") {
          message = `/assets/images/${filename}`;
        }
        socket.broadcast
          .to(to.roomId)
          .emit("message", socket.user, to, message, type, filename, filesize);
      }
      if (to.type === "group") {
        if (type === "file") {
        ...
        store.saveMessage(from, to, message, type, filename, filesize);
      }
    });
